from PyQt6.QtWidgets import QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QWidget, QSizePolicy, QComboBox
from PyQt6.QtCore import Qt
from core.interface.styles.styles import Styles
from core.utils.signals import signal_manager

class DropdownLayout(QWidget):
    def __init__(self, title, description, options, config_id=None, parent=None):
        super().__init__(parent)
        self.config_id = config_id
        self.initUI(title, description, options, config_id)
        
    def initUI(self, title, description, options, config_id):
        ...
        self.dropdown.setStyleSheet(Styles.DROPDOWN)
        self.dropdown.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        ...
        self.dropdown.currentTextChanged.connect(lambda _: signal_manager.value_changed.emit())
    ...
    def set_value(self, value):
        index = self.dropdown.findText(value, Qt.MatchFlag.MatchExactly)
        if index >= 0:
