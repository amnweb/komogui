...
class RuleWidget(QWidget):
    ...
    def initUI(self):
        ...
        # Add rule components
        self.kind = DropdownLayout('Kind', 'Individual window floating rules', 
                                 ["Exe","Class","Title","Path"], 
                                 config_id=f"ignore_rules-{self.index}.kind")
        self.strategy = DropdownLayout('Matching strategy', 'Individual window matching rules',
                                     ["Legacy","Equals","StartsWith","EndsWith","Contains",
                                      "Regex","DoesNotEndWith","DoesNotStartWith",
                                      "DoesNotEqual","DoesNotContain"], 
                                     config_id=f"ignore_rules-{self.index}.matching_strategy")
        self.identifier = InputLayout('ID', 'Identifier as a string', "",
                                    config_id=f"ignore_rules-{self.index}.id")
...
class DynamicLayout(QWidget):
    def __init__(self, parent=None):
        ...
        # Connect signal
        signal_manager.ignore_rules_updated.connect(self.handle_ignore_rules)
        self._initialized = True

    def showEvent(self, event):
        super().showEvent(event)
        
    def handle_ignore_rules(self, rules):
