...
class RuleWidget(QWidget):
    ...
    def initUI(self):
        ...
        # Add rule components
        self.kind = DropdownLayout('Kind', 'Individual applications managed as floating windows', 
                                 ["Exe","Class","Title","Path"], 
                                 config_id=f"floating_applications-{self.index}.kind")
        self.strategy = DropdownLayout('Matching strategy', 'Individual applications matching rules',
                                     ['Legacy','Equals','StartsWith','EndsWith','Contains','Regex','DoesNotEndWith','DoesNotStartWith','DoesNotEqual','DoesNotContain'], 
                                     config_id=f"floating_applications-{self.index}.matching_strategy")
        self.identifier = InputLayout('ID', 'Identifier as a string', "",
                                    config_id=f"floating_applications-{self.index}.id")
...
class DynamicLayout(QWidget):
    def __init__(self, parent=None):
        ...
        # Connect signal
        signal_manager.floating_applications_updated.connect(self.handle_floating_applications)
        self._initialized = True

    def showEvent(self, event):
        super().showEvent(event)
        
    def handle_floating_applications(self, rules):
...
def floating_applications_widget():
